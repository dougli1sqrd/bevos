...
@click.argument("repo")
@click.option("--tag")   # type: ignore # Option descends from Argument, where required is defined
...
def release(owner: str, repo: str, tag: Optional[str], bump: Optional[str], target: str, verbose: bool, dry_run: bool, description: str, token_path: Optional[str], artifact: Optional[str]) -> None:
    ...
    if tag == None and bump == None:
        raise click.UsageError("No release tag defined. Must include --tag TAG or to bump the major, minor, or patch version from the latest with --bump=COMPONENT")

    if tag == None and bump != None:
        tag = ghapi.increment_repo_version(owner, repo, bump, token_path) # type: ignore
    ...
    result = ghapi.perform_release(owner, repo, tag, target, artifact, token_path, description, dry_run) # type: ignore
